...
create (:Actor{actor_id: line[0], first_name: line[1], last_name: line[2]});
...
create (adr)<-[:vive_em]-(:Customer {first_name: line[0], last_name: line[1], email: line[2], active: line[3], create_date: replace(line[4],' ','T')})-[:membro_em]->(st);

load csv from "file:///payment.csv" as line
match (c: Customer {first_name: line[1], last_name: line[2]})
match (s: Staff {username: line[5]})
create (c)<-[:pago_por]-(p:Payment {amount: toFloat(line[3]), payment_date: replace(line[4],' ','T'), payment_id: line[6]})-[:feito_por]->(s);
...
create (sta)<-[:feito_por]-(r:Rental {rental_id: line[0], rental_date: replace(line[1],' ','T'), return_date: replace(line[2], " ", "T")})-[:filme_alugado]->(f),
